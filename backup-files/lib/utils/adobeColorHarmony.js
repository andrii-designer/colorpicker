...
const HARMONY_ANGLES = {
  ...
  COMPOUND: [0, 70, 180, 250], // Similar to split complementary but with more colors
...
/**
 * Generate a harmonious color palette with customizable options
 * @param baseColor Base color hex
 * @param options Options object with type, count, padding
 * @returns Array of hex colors
 */
export function generateHarmoniousPalette(
  baseColor, options = { type: 'complementary', count: 5, padding: 0.1 }
) {
  const { type, count, padding } = options;
  return generateAdobeHarmonyPalette(baseColor, type, count);
}
 ...
 */
function generateAdobeAnalogousPalette(baseColor, count = 5, padding = 30) {
 ...
 */
function generateAdobeMonochromaticPalette(baseColor, count = 5) {
  const tc = tinycolorLib(baseColor);
  const result = [baseColor];
 ...
 */
function generateAdobeTriadPalette(baseColor, count = 5) {
  const tc = tinycolorLib(baseColor);
  const hsl = tc.toHsl();
 ...
 */
function generateAdobeComplementaryPalette(baseColor, count = 5) {
  const tc = tinycolorLib(baseColor);
  const hsl = tc.toHsl();
 ...
 */
function generateAdobeTetradPalette(baseColor, count = 5) {
  const tc = tinycolorLib(baseColor);
  const hsl = tc.toHsl();
 ...
 */
function generateAdobeCompoundPalette(baseColor, count = 5) {
  const tc = tinycolorLib(baseColor);
  const hsl = tc.toHsl();
 ...
 */
function generateAdobeShadesPalette(baseColor, count = 5) {
  const tc = tinycolorLib(baseColor);
  const result = [baseColor];
