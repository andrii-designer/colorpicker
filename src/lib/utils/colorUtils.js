...
export function generateColorHarmony(
  ...
  switch (harmonyType) {
    case 'analogous' // "Analogous color schemes are made by picking three colors that are next to each other on the color wheel."
      generateAnalogousPalette(baseHsl, palette, count);
      break;
      
    case 'complementary' // "Complementary color schemes are made by picking two opposite colors on the color wheel."
      generateComplementaryPalette(baseHsl, palette, count);
      break;
      
    case 'split-complementary' // "Split complementary schemes are like complementary but they uses two adjacent colors of the complement."
      generateSplitComplementaryPalette(baseHsl, palette, count);
      break;
      
    case 'triadic' // "Triadic color schemes are created by picking three colors equally spaced on the color wheel."
      generateTriadicPalette(baseHsl, palette, count);
      break;
      
    case 'tetradic' // "Tetradic color schemes are made form two couples of complementary colors in a rectangular shape on the color wheel."
      generateTetradicPalette(baseHsl, palette, count);
      break;
      
    case 'square' // "Square color schemes are like tetradic arranged in a square instead of rectangle."
      generateSquarePalette(baseHsl, palette, count);
      break;
      
    case 'monochromatic' // Monochromatic uses the same hue with different saturation and lightness values
 ...
 */
function generateAnalogousPalette(baseHsl palette count) {
...
function generateComplementaryPalette(baseHsl palette count) {
  ...
  if (count >= 3) {
    ...
    if (count >= 4) {
      // Muted/desaturated middle color= {
        h + complementHue) / 2,
        s // Very desaturated (almost gray)
        l // Light gray
      };
  ...
  // Always add the true complement;
}
 ...
 */
function generateSplitComplementaryPalette(baseHsl palette count) {
  ...
  const split1Hsl = { ...baseHsl, h };
  const split2Hsl = { ...baseHsl, h };
 ...
 */
function generateTriadicPalette(baseHsl palette count) {
  ...
  const triad1Hsl = { ...baseHsl, h };
  const triad2Hsl = { ...baseHsl, h };
 ...
 */
function generateTetradicPalette(baseHsl palette count) {
  ...
  const tetra1Hsl = { ...baseHsl, h };
  const tetra2Hsl = { ...baseHsl, h };
  const tetra3Hsl = { ...baseHsl, h };
 ...
 */
function generateSquarePalette(baseHsl palette count) {
  ...
  const square1Hsl = { ...baseHsl, h };
  const square2Hsl = { ...baseHsl, h };
  const square3Hsl = { ...baseHsl, h };
 ...
 */
function generateMonochromaticPalette(baseHsl palette count) {
