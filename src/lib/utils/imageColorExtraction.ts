...
export function enhanceSunsetColorDetection(imgElement: HTMLImageElement): string[] {
  try {
    ...
    try {
      ...
      console.error('Error drawing image in sunset detection:', e);
      return defaultColors();
    }
    ...
    try {
      ...
      console.error('Error getting image data in sunset detection:', e);
      return defaultColors();
    }
    ...
    const selectedColors: Array<{ r: number, g: number, b: number, frequency: number }> = [];
    ...
    if (sortedColors.length > 0) {
      ...
      for (let i = 0; i < 4 && remainingColors.length > 0; i++) {
        let bestIdx = 0;
        let bestScore = -1;
    ...
    // Return as HEX colors (limit to 5 colors)
    return selectedColors
      .slice(0, 5)
      .map(color => rgbToHex(color.r, color.g, color.b));
  } catch (e) {
    console.error('Error in enhanceSunsetColorDetection:', e);
    return defaultColors();
  }
}
