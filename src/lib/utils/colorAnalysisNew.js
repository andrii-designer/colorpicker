  ...
  const harmonies=> string[]> = {
    analogous=> {
      ...
      for (let i = 1; i <= Math.floor(count / 2); i++) {
        ...
        const leftColor = tinycolorLib({h s l: hsl.l}).toHexString();
        
        const rightHue = (hsl.h + step * i) % 360;
        const rightColor = tinycolorLib({h s l: hsl.l}).toHexString();
    ...
    triad=> {
      ...
      if (count >= 2) {
        ...
        colors.push(tinycolorLib({h s l: hsl.l}).toHexString());
      }
...
function analyzeHarmony(colors) {
  ...
  if (hueRange > 180) {
    ...
    const adjustedHues = hues.map(h => h > 180 ? h - 360 );
