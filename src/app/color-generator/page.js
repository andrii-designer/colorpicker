...
import { generateColorPalette, regenerateWithLockedColors, Color, PaletteOptions } from '@/lib/utils';
import { ACCURATE_COLOR_DATA } from '@/lib/utils/simplifiedColorData';
...
export default function ColorGenerator() {
  ...
  const analyzeColorPalette = (colors=> {
    if (colors.length < 2) return { score message };
  ...
  const generatePalette = () => {
    ...
    const randomPaletteType = (() => {
      if (Math.random() < 0.4) { // 40% chance to randomize palette type
        const types = ['monochromatic', 'complementary', 'analogous', 'triadic', 'tetradic', 'splitComplementary'];
        return types[Math.floor(Math.random() * types.length)];
      }
      return paletteType;
    })();

    // Create options object with correct typing
    const options= {
      paletteType colorData };

    const newPalette = generateColorPalette(randomizedBaseColor, options);
    ...
    if (newPalette[0] && (newPalette[0]{
      setAnalysisScore((newPalette[0];
      
      // Generate message based on score
      let message = '';
      const score = (newPalette[0];
      ...
      // Use the original analysis function= newPalette.map(c => c.hex);
  ...
  const handleColorSelect = (index=> {
    setSelectedColorIndex(selectedColorIndex === index ? null );
  };
  ...
  const regeneratePaletteWithLocks = () => {
    ...
    const randomizedBaseColor = (() => {
      // Find a non-locked color to use= palette[0].hex;
    ...
    const newPalette = regenerateWithLockedColors(palette, lockedColors, options);
    ...
    <div className="bg-gray-100 min-h-screen p-4 sm:p-8">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Existing configuration options... */}) {/* Color picker */}
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        {/* Analysis score section */}) {analysisScore !== null && (
      ...
      {/* Selected color details */}) {selectedColorIndex !== null && selectedColorIndex < palette.length && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h2 className="text-xl font-bold mb-4">Selected Color Details</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            
              <div className="h-32 rounded-md shadow-sm" style={{ backgroundColor }}></div>
            </div>
