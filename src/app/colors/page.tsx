...
export default function ColorsPage() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'hue' | 'saturation' | 'lightness'>('name');
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-4 mb-6">
        ...
        {STATIC_COLOR_DATA.length < 100 && (
          <ol className="list-decimal pl-5 mb-2 text-sm">
            <li>Run <code className="bg-gray-100 px-1 rounded">node src/scripts/scrapeAllColors.js</code> to scrape all colors</li>
            <li>The script will automatically generate a static color data file</li>
            <li>Restart the server to see all colors on this page</li>
          </ol>
        )}
        
        <p className="text-sm text-blue-700">
          <strong>For production deployment:</strong> The scraper will generate a static TypeScript file with all color data, 
      ...
      <div className="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
        ...
        <div className="mt-4">
          <p className="text-sm text-gray-500">
            Note: This page currently shows the sample color database. To see all 6,000+ colors, 
          ...
          {filteredColors.map((color) => (
            ...
            >
              <div 
                ...
                style={{ backgroundColor: color.hex || `hsl(${color.hue}, ${color.saturation}%, ${color.lightness}%)` }}
              >
                {(color.lightness > 70) ? (
                  ...
                  <span className="px-2 py-1 bg-white bg-opacity-40 text-black text-xs rounded">
                    {color.hex || `HSL(${color.hue}, ${color.saturation}%, ${color.lightness}%)`}
                  </span>
                )}
              </div>
      ...
      <div className="mt-8 p-4 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <ol className="list-decimal pl-5 space-y-2">
          <li>Run the scraper script: <code className="px-2 py-1 bg-gray-100 rounded">node src/scripts/scrapeAllColors.js</code></li>
          <li>Wait for the script to finish (this may take some time as it respects rate limits)</li>
          <li>The color data will be saved to <code className="px-2 py-1 bg-gray-100 rounded">src/lib/utils/colorData.json</code></li>
          <li>Update <code className="px-2 py-1 bg-gray-100 rounded">src/lib/utils/populateColorDatabase.ts</code> to import from the JSON file</li>
