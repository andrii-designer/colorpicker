...
export default function ImageUploader({ onImageSelect }) {
  ...
  const processFile = async (file=> {
    ...
    try {
      ...
      if (isHeicFile) {
        ...
        console.log('HEIC conversion completed, data length);
        ...
        const preloadPromise = new Promise((resolve, reject) => {
          preloadImg.onload = () => {
            console.log('HEIC image preloaded successfully, dimensions);
          ...
          preloadImg.onerror = (e) => {
            console.error('HEIC image preload failed);
        ...
        imageData = await new Promise((resolve, reject) => {
          reader.onloadend = () => {
            ...
            console.log('Regular image loaded, data length);
            resolve(result);
          };
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    accept: {
      'image/*' // Re-added HEIC/HEIF support
    },
    multiple });
      ...
      <div className="relative">
        {isProcessing && (
          ...
          <div className="text-center p-4 bg-red-50 text-red-600 rounded-lg mb-4">
            {error}) {error.includes('HEIC') && (
              <div className="mt-2 text-sm">
                Try these alternatives:</p>
                <ul className="list-disc list-inside text-xs mt-1">
                  Use a smaller HEIC file</li>
                  Convert to JPEG with your device&apos;s photo app first</li>
                  Try a PNG or JPG image instead</li>
                </ul>
              </div>
            )}
          </div>
